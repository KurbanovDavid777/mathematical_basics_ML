...
На практике используется приближённая обратная матрица Гессе:
...
\[
f(x) = \frac{1}{2} \left[ x_1^2 + \sum_{i=1}^{2} (x_i - x_{i+1})^2 + x_3^2 \right] - x_1, \quad x \in \mathbb{R}^3
\]
...
### Description
This laboratory focuses on unconstrained optimization using numerical methods. The main emphasis is placed on quasi-Newton methods (BFGS and L-BFGS) and numerical differentiation techniques.
